...
static volatile int fg_pid = 0; //Keeps track of the pid of the foreground job
static volatile int fg_finished = 1; //Keeps track of whether or not the foreground has finished
...
void eval(char *cmdline) 
{
  ...
  pid_t pid;            /* useful for keeping track of the current pid*/
  sigset_t blocked;     /* keeps track of blocked signals */
  ...
  //Try to execute built-in commands. If not, prepare to call execve.
  if (!builtin_cmd(argv1)){
    ...
    if ((pid = fork()) == 0) {
      ...
      //Make each child part of its own process group
      setpgid(0,0);

      //Attempt to read the command, giving a message if not found.
    ...
    //Wait for the foreground process to complete/change state
    waitfg(pid);
  }
...
void do_bg(int jid) 
{
  ...
  //Find the job by the jid, if it exists
  if(getjobjid(jobs, jid) == NULL){
    printf("%%%d: No such job\n", jid);
    return;
  }
  else{
    job_bg = getjobjid(jobs, jid);
  }

  //Send SIGCONT to the job and its children
  int job_pid = job_bg->pid;
  kill(-job_pid, SIGCONT);

  //Change state to background
  job_bg->state = BG;
  printf("[%d] (%d) %s", job_bg->jid, job_bg->pid, job_bg->cmdline);
  return;
}
...
void do_fg(int jid) 
{
  ...
  if(getjobjid(jobs, jid) == NULL){
    ...
    job_fg = getjobjid(jobs, jid);
  }

  //Send SIGCONT to the job and its children
  int job_pid = job_fg->pid;
  kill(-job_pid, SIGCONT);

  //Change state to foreground, updating fg_pid and fg_finished
  job_fg->state = FG;
  fg_pid = job_pid;
  fg_finished = 0;

  //Wait for the process to complete/change state
  waitfg(job_pid);
  return;
}
...
void sigchld_handler(int sig) 
{
  ...
  while ((finished_pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
    ...
    //Used for SIGTSTP signals
    if (WIFSTOPPED(status)){
      getjobpid(jobs, finished_pid)->state = ST;
...
void sigint_handler(int sig) 
{
  kill(-fg_pid, SIGINT); //Send SIGINT to foreground job and its children
  return;
}
...
void sigtstp_handler(int sig) 
{
  kill(-fg_pid, SIGTSTP); //Send SIGTSTP to foreground job and its children
  return;
}
