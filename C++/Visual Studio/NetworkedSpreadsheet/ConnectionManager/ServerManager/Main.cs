    ...
    public partial class Main : Form
    {
            ...
            while (x == 1)
            {
                if (Networking.isConnected == true)
                {
                    ...
                    try
                    {
                        ...
                        bool check1 = Networking.GetMaster.GetSocket.Poll(5000, SelectMode.SelectRead);
        ...
        private void resetLists(object sender, ElapsedEventArgs e)
        {
            lock (allUsers)
            {
                ...
                ServerStatus.Invoke(new MethodInvoker(delegate { allSpreads.Items.Clear(); }));
                ServerStatus.Invoke(new MethodInvoker(delegate { allUsers.Items.Clear(); }));
            }
        }
                ...
                else
                {
                    ...
                    string[] splitMessages = dataIN.ToString().Split('\n');
                        foreach (string x in splitMessages)
                        {
                            ...
                            else if (x[0] == 'A' && x[x.Length - 1] == ';')
                            {
                                string[] listOfUsers = x.Split(',');
                                //ServerStatus.Invoke(new MethodInvoker(delegate { Users.Items.Clear(); }));
                                foreach (string userAllIN in listOfUsers)
                                {
                                    if (userAllIN == "A" || userAllIN == "" || userAllIN.Contains(".sprd") || userAllIN == ";")
                                    {

                                    }
                                    else
                                    {
                                        if (allUsers.Items.Contains(userAllIN))
                                        {
                                            //Do nothing
                                        }
                                        else
                                        {
                                            ServerStatus.Invoke(new MethodInvoker(delegate { allUsers.Items.Add(userAllIN); }));
                                        }
                                    }
                                }
                                allUsers.Sorted = true;
                            }
                            else if (x[0] == 'Q' && x[x.Length - 1] == ';')
                            {
                                string[] listOfSheets = x.Split(',');
                                //ServerStatus.Invoke(new MethodInvoker(delegate { Spreadsheets.Items.Clear(); }));
                                foreach (string allSheets in listOfSheets)
                                {
                                    if (allSheets == "Q" || allSheets == "" || allSheets == ";" || allSheets.Contains(".sprd") != true)
                                    {
                                        //Don't add
                                    }
                                    else
                                    {
                                        if (allSpreads.Items.Contains(allSheets))
                                        {
                                            //Do nothing
                                        }
                                        else
                                        {
                                            ServerStatus.Invoke(new MethodInvoker(delegate { allSpreads.Items.Add(allSheets); }));
                                        }
                                    }
                                }
                                allSpreads.Sorted = true;
                            }
                            else
                            {
        ...
        private void allUsers_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                string UserName = allUsers.SelectedItem.ToString();
        ...
        private void allSpreads_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                string spreadName = allSpreads.SelectedItem.ToString();
                ServerManager.ModifySheet newModifySheet = new ModifySheet(spreadName);
                newModifySheet.Show();
            }
            catch
            {
