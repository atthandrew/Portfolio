    ...
    public class AccountController : CommonController
    {
        ...
        public string CreateNewUser(string fName, string lName, DateTime DOB, string SubjectAbbrev, string role)
        {
            var maxS = (from s in db.Students
                        ...
                        select s).Take(1);
            int maxSN = maxS.Count() == 0 ? 0 : int.Parse(maxS.First().UserId.Substring(1));

            var maxP = (from p in db.Professors
                        orderby p.UserId descending
                        select p).Take(1);
            int maxPN = maxP.Count() == 0 ? 0 : int.Parse(maxP.First().UserId.Substring(1));

            var maxA = (from a in db.Administrators
                        orderby a.UserId descending
                        select a).Take(1);
            int maxAN = maxA.Count() == 0 ? 0 : int.Parse(maxA.First().UserId.Substring(1));

            int max = Math.Max(maxSN, Math.Max(maxPN, maxAN));
            max++;

            var uid = "u" + max.ToString().PadLeft(7, '0');

            switch (role)
            {
                case "Student":
                    db.Add(new Students { UserId = uid, FName = fName, LName = lName, Dob = DOB, DSubject = SubjectAbbrev });
                    break;

                case "Professor":
                    db.Add(new Professors { UserId = uid, FName = fName, LName = lName, Dob = DOB, Dept = SubjectAbbrev });
                    break;

                case "Administrator":
                    db.Add(new Administrators { UserId = uid, FName = fName, LName = lName, Dob = DOB});
                    break;
            }
